...
export default function CheckSecurityPatchPage() {
    ...
    const handleRecheck = async (item) => {
        ...
        try {
            const response = await fetch('/api/operations/aws/prod/check-security-patch', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    const loadHistory = async (customPage = page, customPageSize = pageSize) => {
        try {
            ...
            const response = await fetch(`/api/operations/aws/prod/check-security-patch?page=${customPage}&limit=${customPageSize}`,
                { cache: 'no-store' }
            );
            const data = await response.json();
    ...
    const exportHistoryToXlsx = async () => {
        try {
            ...
            const response = await fetch('/api/operations/aws/prod/check-security-patch?limit=100&format=xlsx', {
                cache: 'no-store'
            });
        ...
        <div className="max-w-7xl mx-auto space-y-6">
            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                <div className="border-b border-gray-200 pb-4 mb-6">
                    <h1 className="text-2xl font-bold text-gray-900">Check Security Patch (Prod)</h1>
                    <p className="text-gray-600 mt-1">
                        สแกนสถานะ security patch ของ EC2 ใน AWS Production ผ่าน profile <span className="font-mono">aws_prod</span>
                ...
                <div className="grid gap-6 lg:grid-cols-[1.3fr_1fr]">
                    <div className="space-y-4">
                        <div className="rounded-lg border border-green-200 bg-green-50 p-4">
                            ...
                            <p className="text-sm text-gray-600 mb-4">
                                ดึงรายการ EC2 ที่สถานะ <span className="font-mono">running</span> จาก profile <span className="font-mono">aws_prod</span> แล้วเช็ก security patch ทีละ IP
                            </p>
                            <button
                                type="button"
                        ...
                        <div className="rounded-lg border border-blue-200 bg-blue-50 p-4">
                            ...
                            <p className="text-sm text-gray-600 mb-4">
                                ป้อน IP ที่ต้องการ แล้วระบบจะพยายาม map กับ EC2 ใน profile <span className="font-mono">aws_prod</span> ก่อนทำการเช็ก
                            </p>
                    ...
                    {/* <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
                        ...
                        <p className="text-sm text-gray-600 mb-3">
                            ทุกครั้งที่สแกนสำเร็จหรือไม่สำเร็จ ระบบจะ append ลงไฟล์ <span className="font-mono">scan_security_patch_prod.log</span>
                        </p>
