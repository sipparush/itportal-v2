...
export async function POST(request) {
    try {
        ...
        const createServiceCommand = `curl -s -i -X POST https://kong-admin.jfin.network/services --data name=${serviceName} --data url='${targetUrl}'`;

        // 2. Create Route Command
        const createRouteCommand = `curl -s -i -X POST https://kong-admin.jfin.network/services/${serviceName}/routes --data "hosts[]=${domain}" --data "paths[]=${path}" --data "preserve_host=true" --data "name=${routeName}"`;
